...
use crate::{
    ...
    green::Green,
    logging::{self},
    md::{get_or_read, Md, MdId},
    rustc_wrapper::{cargo_home, ENV_EXECUTE_BUILDRS},
...
async fn do_exec_buildrs(
    ...
    let code_stage = previous_md.code_stage();
    if !code_stage.starts_with("cratesio-") {
        panic!(">>> unhandled code_stage {code_stage}")
    }
    let name_and_version = code_stage.as_str().trim_start_matches("cratesio-");
    let code_src = format!("/{name_and_version}");
    let code_dst = cargo_home
        .join("registry/src")
        .join(format!("index.crates.io-0000000000000000/{name_and_version}"));
    let krate_manifest_dir = env::var("CARGO_MANIFEST_DIR").expect("$CARGO_MANIFEST_DIR");
    let krate_manifest_dir = rewrite_cratesio_index(Utf8Path::new(&krate_manifest_dir));
    if code_dst != krate_manifest_dir {
        panic!(">>> code_dst!=krate_manifest_dir : {code_dst}!={krate_manifest_dir}")
    }
    let code_dst = krate_manifest_dir;
    ...
    let call = format!("{ENV_EXECUTE_BUILDRS}= {exe}");
    md.run_block(
        &run_stage,
        &out_stage,
        &out_dir_var,
