...
use crate::{
    ...
    envs::{self, internal, maybe_log, pass_env, syntax, this},
...
"#,
    ...
    let dockerfile = {
        ...
        let syntax = syntax(&green).await.trim_start_matches("docker-image://");
    ...
    let res = build(&green, &dockerfile, out_stage, &md.contexts, Some(&out_dir)).await;
    if let Some(incremental) = res.is_ok().then_some(incremental).flatten() {
        let _ = build(&green, &dockerfile, incremental_stage, &md.contexts, Some(&incremental))
            .await
