...
async fn do_wrap_rustc(
    ...
    md.push_block(
        ...
        green.image.base_image_inline.clone().expect(RUST),
    );
...
"#,
    ...
    let dockerfile = {
        ...
        header.push('\n');
        header.push_str(md.rust_stage().script.trim());
        header.push('\n');
        header.push('\n');
    ...
    let build =
        |stage, dir| build(&green, Network::None, &dockerfile, stage, &md.contexts, Some(dir));
