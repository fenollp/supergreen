...
pub(crate) struct Green {
    ...
    /// Snapshot of all envs. Read once at first cargo invocation.
    #[doc(hidden)]
    pub(crate) env: Vars,
...
impl Green {
    ...
    /// Read an environment variable's value we snapshotted at the start
    pub(crate) fn env(&self, name: &str) -> Option<&str> {
        self.env.get(name).map(String::as_str)
    }

    /// Read an environment variable's value that's reserved for the `Runner`
    pub(crate) fn runner_env(&self, name: &str) -> Option<&str> {
        Runner::envs(&self.env)
            .into_iter()
            .filter_map(|(k, v)| (k == name).then_some(v.as_str()))
            .next()
    }

    /// Reads the out dir path from env
    pub(crate) fn out_dir(&self) -> Option<&Utf8Path> {
        self.env(OUT_DIR!()).map(Into::into)
    }

    /// get_mut for green.cache's fields
    fn cache_setting_from_env(&mut self, name: &str) -> &mut Vec<ImageUri> {
        match name {
            CARGOGREEN_CACHE_FROM_IMAGES!() => &mut self.cache.from_images,
            CARGOGREEN_CACHE_TO_IMAGES!() => &mut self.cache.to_images,
            CARGOGREEN_CACHE_IMAGES!() => &mut self.cache.images,
            _ => unreachable!(),
        }
    }
    ...
    pub(crate) async fn new_from_env_then_manifest(is_install: bool, env: Vars) -> Result<Self> {
        ...
        Self::try_new(manifest, env).map_err(|e| anyhow!("Failed reading {PKG} configuration: {e}"))
    }

    fn try_new(manifest: Manifest, env: Vars) -> Result<Self> {
        ...
        for var in [
            ...
            if let Some(val) = green.env(var) {
                ...
                let val = val
                    ...
                    .collect::<Result<_>>()?;
                let field = green.cache_setting_from_env(var);
                *field = val;
            }
            let field = green.cache_setting_from_env(var);
        ...
        for (field, var) in [
            ...
            let origin = validate_csv(field, var, &green.env)?;
        ...
        validate_csv(&mut green.components, CARGOGREEN_COMPONENTS!(), &green.env)?;
        ...
        validate_csv(&mut green.set_envs, CARGOGREEN_SET_ENVS!(), &green.env)?;
...
"#;
    ...
    fn misspelled_variables_are_reported() {
        ...
        let mut unknowns = find_unknowns(&env);
        unknowns.sort();
