...
impl Green {
    ...
    pub(crate) fn maybe_write_final_path(
        ...
        if self.finalpathcomments() {
            sys().fs.copy(containerfile, path)?;
            ...
            sys().fs.write(path, &strip_comments(&whole))?;
        }

        let call = call.replace(self.paths.cwd.as_str(), "$PWD");
        sys().fs.append(path, &render_reproducer(contexts, &call, envs))?;
        Ok(())
    }
    ...
    pub(crate) fn maybe_append_to_final_path(
        ...
        sys().fs.append(path, &render_trailing_stage(md.as_deref(), &final_stage))?;
        Ok(())
    }
}
