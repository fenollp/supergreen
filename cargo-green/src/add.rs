...
impl Add {
    ...
    pub(crate) fn is_empty(&self) -> bool {
        let Self { apk, apt } = self;
        [apk, apt].iter().all(|x| x.is_empty())
    }

    /// Sums, sorts and dedupes multiple instances
    #[must_use]
    pub(crate) fn union(self, lhs: &Self) -> Self {
        let Self { mut apk, mut apt } = self;
...
RUN \
 ...
 && if command -v apk >/dev/null 2>&1; then \
      ...
      xx-apt-get update && DEBIAN_FRONTEND=noninteractive xx-apt-get satisfy --no-install-recommends -y '{apt}'; \
    fi
"#,
            last = last.trim(),
...
fn quote_pkgs(pkgs: &[String]) -> String {
    if pkgs.is_empty() { "<none>".to_owned() } else { pkgs.join("' '") }
}
