...
pub(crate) async fn as_stage(mdid: MdId, pwd: &Utf8Path) -> Result<NamedStage> {
    info!(
        "mounting {}files under {pwd}",
        if sys().fs.is_dir(&pwd.join(".git")) { "git " } else { "" }
    );

    let (keep, lose) = {
        let mut entries =
            sys().fs.read_dir(pwd).map_err(|e| anyhow!("Failed reading dir {pwd:?}: {e}"))?;
        ...
        entries.into_iter().partition(|fname| {
            ...
            if fname == ".git" && sys().fs.is_dir(&pwd.join(fname)) {
                debug!("excluding {fname} dir");
                return false; // Skip copying .git dir
            }
            if sys().fs.exists(&pwd.join(fname).join("CACHEDIR.TAG")) {
                debug!("excluding {fname} dir");
