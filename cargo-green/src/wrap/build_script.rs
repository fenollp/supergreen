...
#[cfg(test)]
mod pipeline {
    use super::{Green, exec_build_script};
    use crate::{
...
"#
    ...
    /// [`Green::call_config`] tells the two phases apart.
    fn vars() -> Vars {
        [
    ...
    fn run(experiments: &[&str]) -> (String, String) {
        ...
        let builds = FakeBuilds::set_and_wrote([("PROC_MACRO2_SPAN", "1")], ["out/generated.rs"]);
        let _guard = Sys::install(Sys { fs: fs.clone(), builds: builds.clone(), ..Sys::fake() });

        tokio::runtime::Builder::new_current_thread()
            .build()
            .unwrap()
            .block_on(exec_build_script(green(experiments), EXE.into()))
            .unwrap();
