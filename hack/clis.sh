...
((i+=1)); nvs[i]=cargo-audit@0.21.0-pre.0;    oks[i]=ko; nvs_args[i]='--features=fix' # TODO: re-ok when GitHub Actions runners update to patched BuildKit
((i+=1)); nvs[i]=cargo-bpf@2.3.0;             oks[i]=ko; nvs_args[i]='' # Package libelf was not found in the pkg-config search path.
((i+=1)); nvs[i]=cargo-deny@0.16.1;           oks[i]=ko; nvs_args[i]='' # TODO: re-ok when GitHub Actions runners update to patched BuildKit
...
((i+=1)); nvs[i]=zed@main;                    oks[i]=ko; nvs_args[i]='--git https://github.com/zed-industries/zed.git --tag=v0.149.5 zed' # BUG: bad cratesio name [cratesio-curl-sys-0.4.74-curl-8.9.0-index.crates.io-6f17d22bba15001f 1/2] ADD --chmod=0664 --checksum=sha256:8af10b986114528fcdc4b63b6f5f021b7057618411046a4de2ba0f0149a097bf   https://static.crates.io/crates/curl-sys-0.4.74+curl/curl-sys-0.4.74+curl-8.9.0.crate /crate
