...
cat <<EOF
    ...
    - if: \${{ always() && env.CARGOGREEN_FINAL_PATH != '' }}
      ...
      run: |
        ...
        final_diff+=(--ignore-matching-lines='^# Generated by')
        final_diff+=(--ignore-matching-lines='"autocfg_')
        final_diff+=(--ignore-matching-lines=' AS rust-base$')
        final_diff+=(--ignore-matching-lines='^# syntax=docker.io/docker/dockerfile:1@')
