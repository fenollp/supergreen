...
stable=1.96.0 # Closest to latest stable, as official Rust images availability permits (TODO: use rustup when image isn't yet available)
...
action__docker_login='docker/login-action@371161bbe7024a29a25c5e19bfcbc0804fe9ad2c # v4.5.2'
...
check_bin_hash() {
    ...
    [[ $# -eq 0 ]]
cat <<EOF
    - name: Check that generated binary matches sha256
      run: |
        [[ "\$BINHASHED" = "\$(sha256sum $root/bin/$binname)" ]]
EOF
}
...
postconds() {
    ...
    [[ $# -eq 0 ]]
cat <<EOF
    - if: \${{ job.steps.do-try.outcome != 'success' && job.steps.do-try-jobs1.outcome != 'success' }}
